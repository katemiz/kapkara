import { au as component_context, aL as legacy_mode_flag, H as user_effect, u as untrack, d as push, w as spread_props, x as comment, m as first_child, b as append, c as pop, y as rest_props, z as noop, aM as commonjsGlobal, L as getDefaultExportFromCjs, V as isEqual$1, a3 as cloneDeep, a0 as merge, p as prop, t as template_effect, g as sibling, j as from_html, k as child, l as set_text, r as state, A as proxy, a as if_block, n as delegated, f as setup_stores, o as set, q as get, h as store_mutate, s as store_get, v as delegate, C as each, D as index, F as user_derived } from "./app-DmZvep12.js";
import { b as bind_this } from "./this-CCnJv5FU.js";
import { s as set_class, a as set_attribute } from "./attributes-B9LAyNIy.js";
...
function Upload($$anchor, $$props) {
  ...
  Icon($$anchor, spread_props({ name: "upload" }, () => props, {
    get iconNode() {
      return iconNode;
    },
...
function makeAttachedBlot(node, scroll) {
  ...
  try {
    ...
    return Array.from(node.childNodes).forEach((child2) => {
      blot.domNode.appendChild(child2);
...
const _ParentBlot = class _ParentBlot2 extends ShadowBlot {
  ...
  attach() {
    super.attach(), this.children.forEach((child2) => {
      child2.attach();
    });
  }
  attachUI(node) {
  ...
  build() {
    this.children = new LinkedList(), Array.from(this.domNode.childNodes).filter((node) => node !== this.uiNode).reverse().forEach((node) => {
      try {
        const child2 = makeAttachedBlot(node, this.scroll);
        this.insertBefore(child2, this.children.head || void 0);
      } catch (err) {
  ...
  deleteAt(index2, length) {
    if (index2 === 0 && length === this.length())
      return this.remove();
    this.children.forEachAt(index2, length, (child2, offset, childLength) => {
      child2.deleteAt(offset, childLength);
    });
  }
  descendant(criteria, index2 = 0) {
    const [child2, offset] = this.children.find(index2);
    return criteria.blotName == null && criteria(child2) || criteria.blotName != null && child2 instanceof criteria ? [child2, offset] : child2 instanceof _ParentBlot2 ? child2.descendant(criteria, offset) : [null, -1];
  }
  descendants(criteria, index2 = 0, length = Number.MAX_VALUE) {
    ...
    return this.children.forEachAt(
      index2,
      length,
      (child2, childIndex, childLength) => {
        (criteria.blotName == null && criteria(child2) || criteria.blotName != null && child2 instanceof criteria) && descendants.push(child2), child2 instanceof _ParentBlot2 && (descendants = descendants.concat(
          child2.descendants(criteria, childIndex, lengthLeft)
  ...
  enforceAllowedChildren() {
    ...
    this.children.forEach((child2) => {
        ...
        (def) => child2 instanceof def
      ) || (child2.statics.scope === Scope.BLOCK_BLOT ? (child2.next != null && this.splitAfter(child2), child2.prev != null && this.splitAfter(child2.prev), child2.parent.unwrap(), done = true) : child2 instanceof _ParentBlot2 ? child2.unwrap() : child2.remove());
    });
  }
  formatAt(index2, length, name, value) {
    this.children.forEachAt(index2, length, (child2, offset, childLength) => {
      child2.formatAt(offset, childLength, name, value);
    });
  }
  insertAt(index2, value, def) {
    const [child2, offset] = this.children.find(index2);
    if (child2)
      child2.insertAt(offset, value, def);
    else {
  ...
  length() {
    return this.children.reduce((memo, child2) => memo + child2.length(), 0);
  }
  moveChildren(targetParent, refNode) {
    this.children.forEach((child2) => {
      targetParent.insertBefore(child2, refNode);
    });
  }
  optimize(context) {
    if (super.optimize(context), this.enforceAllowedChildren(), this.uiNode != null && this.uiNode !== this.domNode.firstChild && this.domNode.insertBefore(this.uiNode, this.domNode.firstChild), this.children.length === 0)
      if (this.statics.defaultChild != null) {
        const child2 = this.scroll.create(this.statics.defaultChild.blotName);
        this.appendChild(child2);
      } else
        this.remove();
  }
  path(index2, inclusive = false) {
    const [child2, offset] = this.children.find(index2, inclusive), position = [[this, index2]];
    return child2 instanceof _ParentBlot2 ? position.concat(child2.path(offset, inclusive)) : (child2 != null && position.push([child2, offset]), position);
  }
  removeChild(child2) {
  ...
  split(index2, force = false) {
    ...
    return this.parent && this.parent.insertBefore(after, this.next || void 0), this.children.forEachAt(index2, this.length(), (child2, offset, _length) => {
      const split = child2.split(offset, force);
      split != null && after.appendChild(split);
    }), after;
  }
  splitAfter(child2) {
    const after = this.clone();
    for (; child2.next != null; )
      after.appendChild(child2.next);
...
const _InlineBlot = class _InlineBlot2 extends ParentBlot$1 {
  ...
  format(name, value) {
    if (name === this.statics.blotName && !value)
      this.children.forEach((child2) => {
        child2 instanceof _InlineBlot2 || (child2 = child2.wrap(_InlineBlot2.blotName, true)), this.attributes.copy(child2);
      }), this.unwrap();
    else {
...
const EmbedBlot$1 = EmbedBlot, OBSERVER_CONFIG = {
  ...
  deleteAt(index2, length) {
    this.update(), index2 === 0 && length === this.length() ? this.children.forEach((child2) => {
      child2.remove();
    ...
    for (let i = 0; remaining.length > 0; i += 1) {
      ...
      for (remaining.forEach((mutation) => {
        ...
        blot != null && (blot.domNode === mutation.target && (mutation.type === "childList" ? (mark(this.find(mutation.previousSibling, false)), Array.from(mutation.addedNodes).forEach((node) => {
          const child2 = this.find(node, false);
          mark(child2, false), child2 instanceof ParentBlot$1 && child2.children.forEach((grandChild) => {
...
function requireLodash_clonedeep() {
  ...
  (function(module, exports$1) {
    ...
    var DataView = getNative(root2, "DataView"), Map = getNative(root2, "Map"), Promise2 = getNative(root2, "Promise"), Set = getNative(root2, "Set"), WeakMap2 = getNative(root2, "WeakMap"), nativeCreate = getNative(Object, "create");
...
function requireLodash_isequal() {
  ...
  (function(module, exports$1) {
    ...
    var Buffer = moduleExports ? root2.Buffer : void 0, Symbol2 = root2.Symbol, Uint8Array = root2.Uint8Array, propertyIsEnumerable = objectProto.propertyIsEnumerable, splice = arrayProto.splice, symToStringTag = Symbol2 ? Symbol2.toStringTag : void 0;
    var nativeGetSymbols = Object.getOwnPropertySymbols, nativeIsBuffer = Buffer ? Buffer.isBuffer : void 0, nativeKeys = overArg(Object.keys, Object);
    var DataView = getNative(root2, "DataView"), Map = getNative(root2, "Map"), Promise2 = getNative(root2, "Promise"), Set = getNative(root2, "Set"), WeakMap2 = getNative(root2, "WeakMap"), nativeCreate = getNative(Object, "create");
    var dataViewCtorString = toSource(DataView), mapCtorString = toSource(Map), promiseCtorString = toSource(Promise2), setCtorString = toSource(Set), weakMapCtorString = toSource(WeakMap2);
    var symbolProto = Symbol2 ? Symbol2.prototype : void 0, symbolValueOf = symbolProto ? symbolProto.valueOf : void 0;
...
let Editor$1 = class Editor {
  ...
  insertText(index2, text) {
    ...
    return this.update(new Delta().retain(index2).insert(text, cloneDeep(formats)));
  }
  isBlank() {
...
function convertListHTML(items, lastIndent, types) {
  ...
  if (indent > lastIndent) {
    ...
    if (indent === lastIndent + 1) {
      return `<${tag}><li${attribute}>${convertHTML(child2, offset, length)}${convertListHTML(rest, indent, types)}`;
    }
    return `<${tag}><li>${convertListHTML(items, lastIndent + 1, types)}`;
  }
  const previousType = types[types.length - 1];
  if (indent === lastIndent && type === previousType) {
    return `</li><li${attribute}>${convertHTML(child2, offset, length)}${convertListHTML(rest, indent, types)}`;
  }
...
function convertHTML(blot, index2, length) {
  ...
  if (blot instanceof ParentBlot$1) {
    if (blot.statics.blotName === "list-container") {
      const items = [];
      blot.children.forEachAt(index2, length, (child2, offset, childLength) => {
        const formats = "formats" in child2 && typeof child2.formats === "function" ? child2.formats() : {};
        items.push({
          child: child2,
          offset,
          length: childLength,
    ...
    const parts = [];
    blot.children.forEachAt(index2, length, (child2, offset, childLength) => {
      parts.push(convertHTML(child2, offset, childLength));
    });
...
class Scroll extends ScrollBlot$1 {
  ...
  lines() {
    ...
    const getLines = (blot, blotIndex, blotLength) => {
      ...
      blot.children.forEachAt(blotIndex, blotLength, (child2, childIndex, childLength) => {
        if (isLine$1(child2)) {
          lines.push(child2);
        } else if (child2 instanceof ContainerBlot$1) {
          lines = lines.concat(getLines(child2, childIndex, lengthLeft));
        }
        lengthLeft -= childLength;
      });
      return lines;
...
class CodeBlockContainer extends Container {
  ...
  code(index2, length) {
    return this.children.map((child2) => child2.length() <= 1 ? "" : child2.domNode.innerText).join("\n").slice(index2, index2 + length);
  }
  html(index2, length) {
    return `<pre>
...
`;
    ...
    if (forced || this.forceNext || this.cachedText !== text) {
      if (text.trim().length > 0 || this.cachedText == null) {
        const oldDelta = this.children.reduce((delta2, child2) => {
          return delta2.concat(blockDelta(child2, false));
        }, new Delta());
...
class TableRow extends Container {
  ...
  optimize(context) {
    super.optimize(context);
    this.children.forEach((child2) => {
      if (child2.next == null) return;
      const childFormats = child2.formats();
      const nextFormats = child2.next.formats();
      if (childFormats.table !== nextFormats.table) {
        const next = this.splitAfter(child2);
        if (next) {
          next.optimize();
        }
        if (this.prev) {
...
var root$1 = from_html(`<div><div></div> <p> </p></div>`);
function Editor2($$anchor, $$props) {
  push($$props, true);
  let placeholder = prop($$props, "placeholder", 3, "Write something...");
  let editorElement;
  let quill;
  onMount(() => {
    ...
    input.onchange = async () => {
      ...
      if (file) {
        ...
        try {
          ...
          const response = await axios.post("/upload-image", formData, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  var $$exports = { getContent };
  var div = root$1();
  var div_1 = child(div);
  bind_this(div_1, ($$value) => editorElement = $$value, () => editorElement);
  var p = sibling(div_1, 2);
  var text = child(p);
  template_effect(() => set_text(text, editorElement));
  append($$anchor, div);
  return pop($$exports);
}
var root_2 = from_html(`<span class="has-text-danger">*</span>`);
var root_3 = from_html(`<span class="has-text-grey is-size-7"> </span>`);
var root_1 = from_html(`<label class="label"> <!> <!></label>`);
var root_4 = from_html(`<span class="file-name"> </span>`);
var root_6 = from_html(`<p class="has-text-weight-semibold mb-2"> </p>`);
var root_7 = from_html(`<div class="box py-2 px-3 mb-2"><div class="is-flex is-justify-content-space-between is-align-items-center"><div><p class="is-size-7 has-text-weight-semibold"> </p> <p class="is-size-7 has-text-grey"> </p></div> <button type="button" class="delete is-small" title="Remove"></button></div></div>`);
var root_5 = from_html(`<div class="mt-3"><!> <!></div>`);
var root_9 = from_html(`<div class="column is-one-quarter-desktop is-one-third-tablet is-half-mobile"><figure class="image is-square"><img style="object-fit: cover;"/></figure></div>`);
var root_8 = from_html(`<div class="mt-3"><p class="has-text-weight-semibold mb-2">Preview:</p> <div class="columns is-multiline"></div></div>`);
var root_10 = from_html(`<button type="button" class="button is-small is-light mt-2"> </button>`);
var root_11 = from_html(`<p class="help is-danger"> </p>`);
var root_14 = from_html(`<p class="help is-danger"> </p>`);
var root = from_html(`<div><!> <div class="control"><div><label class="file-label"><input class="file-input" type="file"/> <span class="file-cta svelte-w0gny0"><span class="file-icon"><!></span> <span class="file-label"> </span></span> <!></label></div></div> <!> <!> <!> <!></div>`);
function FormUpload($$anchor, $$props) {
  push($$props, true);
  ...
  let fileInput;
  let previewUrls = state(proxy([]));
  let fileNames = state(proxy([]));
  function handleFileChange(event) {
    const files = Array.from(event.target.files || []);
    if (files.length === 0) {
      set(previewUrls, [], true);
      set(fileNames, [], true);
      return;
    }
    if (maxSize()) {
    ...
    if (showPreview()) {
      ...
      files.forEach((file) => {
        if (file.type.startsWith("image/")) {
          const reader = new FileReader();
          reader.onload = (e) => {
            set(previewUrls, [...get(previewUrls), e.target.result], true);
          };
          reader.readAsDataURL(file);
        }
      });
    ...
    if (multiple()) {
      store_mutate($$props.form, untrack($form)[$$props.name] = files, untrack($form));
    } else {
      store_mutate($$props.form, untrack($form)[$$props.name] = files[0], untrack($form));
    }
  }
  ...
  function clearFiles() {
    ...
    set(previewUrls, [], true);
    set(fileNames, [], true);
    store_mutate($$props.form, untrack($form)[$$props.name] = multiple() ? [] : null, untrack($form));
  }
  function removeFile(index2) {
    ...
    set(previewUrls, get(previewUrls).filter((_, i) => i !== index2), true);
    set(fileNames, get(fileNames).filter((_, i) => i !== index2), true);
    const files = Array.from($form()[$$props.name] || []);
    ...
    var consequent_2 = ($$anchor2) => {
      ...
      var node_2 = sibling(node_1, 2);
      {
        var consequent_1 = ($$anchor3) => {
          ...
          template_effect(() => set_text(text_1, `(Max: ${maxSize() ?? ""}MB)`));
          append($$anchor3, span_1);
        };
        if_block(node_2, ($$render) => {
          if (maxSize()) $$render(consequent_1);
        });
      }
      template_effect(() => {
  ...
  var node_4 = sibling(span_2, 2);
  {
    var consequent_3 = ($$anchor2) => {
      ...
      template_effect(() => set_text(text_3, get(fileNames)[0].name));
      append($$anchor2, span_5);
    };
    if_block(node_4, ($$render) => {
      if (get(fileNames).length > 0 && !multiple()) $$render(consequent_3);
    });
  }
    ...
    var consequent_5 = ($$anchor2) => {
      ...
      var node_6 = child(div_3);
      {
        var consequent_4 = ($$anchor3) => {
          var p = root_6();
          var text_4 = child(p);
          template_effect(() => set_text(text_4, `Selected Files (${get(fileNames).length ?? ""}):`));
          append($$anchor3, p);
        };
        if_block(node_6, ($$render) => {
          if (multiple()) $$render(consequent_4);
        });
      }
      var node_7 = sibling(node_6, 2);
      each(node_7, 17, () => get(fileNames), index, ($$anchor3, file, index2) => {
        var div_4 = root_7();
        ...
        template_effect(() => {
          set_text(text_5, get(file).name);
          set_text(text_6, get(file).size);
          button.disabled = disabled();
        });
        delegated("click", button, () => removeFile(index2));
        append($$anchor3, div_4);
      });
      append($$anchor2, div_3);
    };
    if_block(node_5, ($$render) => {
      if (get(fileNames).length > 0 && showFileInfo()) $$render(consequent_5);
    });
  }
  var node_8 = sibling(node_5, 2);
  {
    var consequent_6 = ($$anchor2) => {
      ...
      each(div_8, 21, () => get(previewUrls), index, ($$anchor3, url, index2) => {
        var div_9 = root_9();
        var figure = child(div_9);
        var img = child(figure);
        set_attribute(img, "alt", `Preview ${index2 + 1}`);
    ...
    if_block(node_8, ($$render) => {
      if (get(previewUrls).length > 0 && showPreview()) $$render(consequent_6);
    });
  }
  var node_9 = sibling(node_8, 2);
  {
    var consequent_7 = ($$anchor2) => {
      ...
      template_effect(() => {
        ...
        set_text(text_7, `Clear ${multiple() ? "All" : "File"}`);
      });
      delegated("click", button_1, clearFiles);
      append($$anchor2, button_1);
    };
    if_block(node_9, ($$render) => {
      if (get(fileNames).length > 0) $$render(consequent_7);
    });
  }
  var node_10 = sibling(node_9, 2);
  {
    var consequent_8 = ($$anchor2) => {
      ...
      template_effect(() => set_text(text_8, $form().errors[$$props.name]));
      append($$anchor2, p_3);
    };
    var alternate = ($$anchor2) => {
      var fragment = comment();
      var node_11 = first_child(fragment);
      each(node_11, 1, () => Object.keys($form().errors), index, ($$anchor3, errorKey) => {
        var fragment_1 = comment();
        var node_12 = first_child(fragment_1);
        {
          var consequent_9 = ($$anchor4) => {
            ...
            template_effect(() => set_text(text_9, $form().errors[get(errorKey)]));
            append($$anchor4, p_4);
          };
          var d = user_derived(() => get(errorKey).startsWith(`${$$props.name}.`));
    ...
    if_block(node_10, ($$render) => {
      if ($form().errors[$$props.name]) $$render(consequent_8);
      else $$render(alternate, -1);
    });
  }
  template_effect(() => {
    set_class(div, 1, `field ${customClass() ?? ""}`, "svelte-w0gny0");
    set_class(div_2, 1, `file has-name ${$form().errors[$$props.name] ? "is-danger" : ""}`, "svelte-w0gny0");
    set_attribute(input, "id", id());
    input.required = required();
    input.disabled = disabled();
    input.multiple = multiple();
    set_attribute(input, "accept", accept());
    set_text(text_2, multiple() ? "Choose files…" : "Choose a file…");
  });
...
//# sourceMappingURL=FormUpload-BB8arxpK.js.map
