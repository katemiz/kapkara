<script>
    ...
    let { asset, permissions, revisions } = $props();
...
<Layout>
    <section class="section min-height-screen">
        <Title
            title="My Assets"
            subtitle="Show Asset Attributes and Properties"
        />
        ...
        <div class="box mt-6">
            <nav class="level is-mobile">
                ...
                <div class="level-left">
                    <p class="buttons">
                        <a href="/mylib" class="button is-ghost">
                        ...
                        <a href="/mylib/create" class="button is-ghost">
                            <span class="icon is-small">
                                <Plus size="20" />
                            </span>
                            <span>Add New</span>
                ...
                <div class="level-right">
                    ...
                    {#if permissions.is_releaseable}
                        <button
                            onclick={() =>
                                confirmFreezeReleaseRevise("release")}
                            class="button is-dark"
            ...
            <div class="columns">
                ...
                <div class="column">
                    ...
                    <Title title={"A" + asset.id} subtitle={asset.title} />
                </div>

                <div class="column has-text-right">
                    <span class="tag"
                        >{assets_config.asset_types.find(
                            (type) => type.value === asset.asset_type,
                        )?.description}</span
                    >
                    <div class="mt-1">
                        {#if revisions}
                            {#each revisions as revision}
                                <a href="/mylib/{revision.id}">
                                    <span
                                        class="tag {asset.revision ==
                                        revision.revision
                                            ? 'is-black'
                                            : 'is-warning'} mr-1"
                                        >R{revision.revision}</span
                                    >
                                </a>
                            {/each}
                        {/if}
            ...
            <RecordData
                item={asset}
                {url}
                status={assets_config.statuses.find(
