...
export default class MastVibration {
    ...
    constructor(options = {}) {
        ...
        this.tipMass = options.tip_mass || 0;
        this.modes = options.modes || 5;
        this.integrationSteps = options.integrationSteps || 500;
        this.sections = [];

        Object.entries(options.tubes).forEach(([key, tube]) => {
            const index = parseInt(key);

            this.sections[index] = {};

            if (index === 0) {
                this.sections[index].L = (options.extendedHeight - options.tubes[index + 1].extended_zt) / 1000;
            } else if (index === options.tubes.length - 1) {
                this.sections[index].L = tube.extended_zt / 1000;
            } else {
                this.sections[index].L = (tube.extended_zt - options.tubes[index + 1].extended_zt) / 1000;
            }
